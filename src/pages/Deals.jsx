...
const stages = [
  { id: 'anticipo', name: 'Anticipo Recibido', color: 'bg-emerald-500' },
  { id: 'production', name: 'En Producción', color: 'bg-blue-500' },
  { id: 'shipment', name: 'Logística / Envío', color: 'bg-purple-500' },
  { id: 'installation', name: 'Instalación', color: 'bg-orange-500' },
  { id: 'completed', name: 'Finalizado', color: 'bg-green-600' },
  { id: 'canceled', name: 'Cancelado', color: 'bg-red-500' },
];
...
const Deals = () => {
  const { theme } = useTheme();
  const { user } = useAuth();
  const { deals, loading, updateDeal, removeDeal } = useData();
  ...
  const handleRevertToLead = useCallback(async (dealToRevert) => {
    ...
    if (leadError) {
      toast({ title: "Error al implementar prospecto", description: leadError.message, variant: "destructive" });
      return;
    }
