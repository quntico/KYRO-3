  ...
  <NavLink
    ...
    className={({ isActive }) =>
      `flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 group ${isActive
        ? 'bg-primary text-primary-foreground shadow-[0_4px_12px_rgba(var(--primary),0.2)] scale-105'
...
const Sidebar = () => {
  ...
  return (
    <>
      {!isMobile && (
        <motion.aside
          className="hidden md:flex flex-col bg-card/75 backdrop-blur-[40px] border-r border-border h-screen fixed left-0 top-0 z-50 shadow-[10px_0_40px_rgba(0,0,0,0.12)]"
          animate={{
            width: isCollapsed ? '5rem' : '16rem',
          }}
          transition={{ duration: 0.5, ease: [0.23, 1, 0.32, 1] }}
        >
          <div className={`flex items-center h-20 border-b border-border transition-all duration-300 ${isCollapsed ? 'justify-center' : 'justify-center'} relative bg-secondary/20`}>
            <KyroRune className={`w-9 h-9 text-primary transition-all duration-300 drop-shadow-[0_0_20px_rgba(var(--primary),0.3)]`} />

            <button
              onClick={() => toggleSidebar()}
              className="absolute -right-3 top-1/2 -translate-y-1/2 z-50 bg-card border border-border p-1.5 text-muted-foreground hover:text-primary rounded-full flex items-center justify-center transition-all hover:scale-110 shadow-md"
            ...
            <AnimatePresence>
              {!isCollapsed && (
                ...
                >
                  <span className="text-2xl font-bold text-foreground tracking-[0.25em] uppercase whitespace-nowrap drop-shadow-sm filter brightness-100">
                    KYRO
                  </span>
                </motion.div>
              )}
          ...
          <div className="p-3 border-t border-border bg-secondary/10 mb-4">
            <NavItem to="/settings" icon={Settings} label="Ajustes" isCollapsed={isCollapsed} />
            <button
              onClick={handleSignOut}
              title={isCollapsed ? "Cerrar Sesión" : ""}
              className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group w-full mt-2 text-red-500 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 ${isCollapsed ? 'justify-center' : ''}`}
            >
