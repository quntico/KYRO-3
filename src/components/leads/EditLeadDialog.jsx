    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
        ...
        <div className="grid gap-6 py-6 max-h-[70vh] overflow-y-auto px-6 scrollbar-hide">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="name" value={editingLead.name} onChange={handleInputChange} className={theme === 'nova' ? 'bg-secondary/40' : ''} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="contact">Contacto</Label>
              <Input id="contact" value={editingLead.contact} onChange={handleInputChange} className={theme === 'nova' ? 'bg-secondary/40' : ''} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="position">Puesto</Label>
              <Input id="position" value={editingLead.position || ''} onChange={handleInputChange} className={theme === 'nova' ? 'bg-secondary/40' : ''} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" value={editingLead.email} onChange={handleInputChange} className={theme === 'nova' ? 'bg-secondary/40' : ''} />
            </div>
            <div className="space-y-2">
                  ...
                  {companies.map((c) => (
                    <SelectItem key={c.id} value={c.id} className="focus:bg-secondary cursor-pointer">
                      <div className="flex items-center gap-2">
                        {c.logo ? (
                          <img src={c.logo} alt={c.name} className="h-4 w-auto max-w-[50px] object-contain rounded" />
                        ) : (
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-2">
              ...
              <Select onValueChange={handleStatusChange} value={editingLead.status}>
                <SelectTrigger id="status" className={theme === 'nova' ? 'bg-white/10' : ''}>
                  <SelectValue placeholder="Selecciona un status" />
                </SelectTrigger>
                <SelectContent className="bg-popover border-border text-popover-foreground">
                  {statusOptions.map(option => (
                    <SelectItem key={option.value} value={option.value} className="focus:bg-secondary cursor-pointer">{option.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              className={`h-24 ${theme === 'nova' ? 'bg-white/10' : ''}`}
            />
          </div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {(() => {
                        if (!followUpDate) return <span>Elige una fecha</span>;
                        try {
                          return format(followUpDate, "PPP");
                        } catch (e) {
      ...
      {calcMachineIndex !== null && (
        <LeadQuoteCalculator
          ...
          location={editingLead?.source}
          clientName={editingLead?.name}
          clientCompany={editingLead?.company}
          machine={editingLead?.machines?.[calcMachineIndex]}
          onApply={(updatedMachine) => {
            setEditingLead(prev => {
              if (!prev) return null;
              const newMachines = [...(prev.machines || [])];
              newMachines[calcMachineIndex] = updatedMachine;
              return { ...prev, machines: newMachines };
            });
          }}
        />
      ...
      {pdfExportMachineIndex !== null && (
        <ExportPdfDialog
          ...
          clientName={editingLead?.name}
          location={editingLead?.source}
          machine={editingLead?.machines?.[pdfExportMachineIndex]}
        />
      )}
    </Dialog>
  );
